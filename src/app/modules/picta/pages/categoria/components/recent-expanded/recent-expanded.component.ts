...
type RecentMedia = 'peliculas' | 'series';

@Component({
  selector: 'app-recent-expanded',
  templateUrl: './recent-expanded.component.html',
  styleUrls: ['./recent-expanded.component.scss'],
...
export class RecentExpandedComponent {
  ...
  readonly media = signal<RecentMedia>('peliculas');
  readonly loading = signal<boolean>(true);
  readonly items = signal<Array<Publication | Serie>>([]);

  readonly isMovieView = computed(() => this.media() === 'peliculas');
  readonly pageTitle = computed(() =>
    this.isMovieView() ? 'Películas recientes' : 'Series recientes',
  );
  readonly pageSubtitle = computed(() =>
    this.isMovieView()
      ? 'Listado de los 20 títulos más recientes de películas.'
      : 'Listado de los 20 títulos más recientes de series.',
  );
  ...
  constructor() {
    this.route.paramMap.subscribe((params) => {
      ...
      const media: RecentMedia = raw === 'series' ? 'series' : 'peliculas';
