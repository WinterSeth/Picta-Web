...
export class CardComponent {
  ...
  readonly externalId = input<string | undefined>(undefined);
  benefits: any;
  plans;
  loading = true;
  ...
  openPayment() {
      const externalId = this.externalId() || `pago_suscripcion_${this.plan().id}`;
  ...
  isPopular(): boolean {
    const name = (this.plan()?.nombre || '').toLowerCase();
    return this.plan()?.id == 40 || name.includes('premium');
  }
  ...
  isActivePlan(): boolean {
    ...
    const current = this.buyed?.();
    return !!current && current.id === this.plan()?.id;
  }

  heroClass(): string {
    if (this.isPopular()) {
      return 'plan-hero-premium';
    }

    const idx = Number(this.index() || 0);
    return idx % 2 === 0 ? 'plan-hero-basic' : 'plan-hero-standard';
  }
